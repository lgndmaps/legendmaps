 ...
 */
export default class UniformImproved extends RotDungeon {
    _options: Options;
    ...
    constructor(reservedX: number, width: number, height: number, seed?: number, options?: Partial<Options>) {
        ...
        this._map = [];
        this._dug = 0;
        this._roomAttempts = 100; /* new room is created N-times until is considered as impossible to generate */
        this._corridorAttempts = 100; /* corridors are tried N-times until the level is considered as impossible to connect */
    ...
    /**
     * PEG: Attempt at smarter room fill
     */
    _generateRoomsFill(cx, cy) {
        let filled: boolean = false;

        while (!filled) {
            let r = this._generateRoomXY(cx, cy);
    ...
    /**
     * PEG: Try to generate one room at set spot
     */
    _generateRoomXY(x: number, y: number) {
        let count = 0;
        while (count < this._roomAttempts) {
            count++;

            let room = Room.createRandomAtNoDoor(x, y, this._width, this._height, this._options);
    ...
    /**
     * Generates additional connectors
     */
    _addExtraCorridors() {
        let cnt = 0;
        ...
        for (let i = 0; i < this._rooms.length; i++) {
            let room = this._rooms[i];
            let compareList: Room[] = [];
            let closestRoom: Room = this._closestRoom(this._rooms, room);
            for (let r = 0; r < this._rooms.length; r++) {
                if (
                    r != i &&
                    this._rooms[r] != closestRoom &&
                    this._rooms[r].getDoorCount() < 3 &&
                    this._rooms[r].getTileSize() > 16
                ) {
                    compareList.push(this._rooms[r]);
                }
            }
            let secondClosest: Room = this._closestRoom(compareList, room);
            let chance: number = 100;
            if (room.getTileSize() < 24) chance = 50;
            if (room.getDoorCount() > 2) chance * 0.5;

            if (
                secondClosest.getDoorCount() < 3 &&
                room.getDoorCount() < 3 &&
                RandomUtil.instance.percentChance(chance)
            ) {
                this._connectRooms(room, secondClosest);
            }
        }
    }
    ...
    _digAStar(start, end) {
        let hallway = new Hallway(this._map, start[0], start[1], end[0], end[1]);
        hallway.create(this._digCallback);
