...
export default class ConditionBleeding extends ConditionData {
    static PERCENT_DAMAGE: number = 0.04;

    constructor() {
        super();
        this.desc = ConditionBleeding.PERCENT_DAMAGE * 100 + "% of max hp dam/turn";
        this.kind = GlobalConst.CONDITION.BLEEDING;
    }

    protected ProcessTurnUpdate(target: EntityLiving, condition: Condition) {
        let damage: number = Math.ceil(target.hpmax * ConditionBleeding.PERCENT_DAMAGE);
        let damageResult: DamageResult = target.doDamage(
            damage,
            GlobalConst.DAMAGE_TYPES.BLADE,
